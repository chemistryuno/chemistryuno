...
export class VibrationEngine {
  ...
  vibrate(pattern: VibrationPattern | VibrationPreset) {
    if (!this.enabled) {
      console.log('[VibrationEngine] 振动已禁用')
      return false
    }
    ...
    try {
      ...
      if (success) {
        console.log('[VibrationEngine] 振动触发成功:', vibrationPattern)
      } else {
  ...
  stop() {
    if (this.isSupported()) {
      navigator.vibrate(0)
      console.log('[VibrationEngine] 已停止所有振动')
    }
  }
